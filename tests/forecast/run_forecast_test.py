...
from pkg.forecast.run_forecast import run_forecast, get_model_checkpoint, build_model, load_normalization_data
...
def test_get_model_checkpoint_success(monkeypatch):
    ...
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.storage.Client",
        mock.Mock(create_anonymous_client=lambda: fake_client),
    )
    # Patch checkpoint.load to return our DummyCKPT
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.checkpoint.load",
        lambda f, cls: DummyCKPT(),
    )

    ckpt, bucket, prefix = get_model_checkpoint("GRAPHCAST")
...
def test_get_model_checkpoint_notfound(monkeypatch):
    ...
    with pytest.raises(FileNotFoundError):
        get_model_checkpoint("NON_EXISTENT_MODEL")
...
def test_load_normalization_data(monkeypatch):
    ...
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.xr.load_dataset",
        lambda f: ds,
    )
    ...
    diffs, mean, std = load_normalization_data(fake_bucket, "myprefix/")
...
def test_build_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.graphcast.GraphCast",
        lambda mc, tc: "GRAPHCAST_INSTANCE",
    )
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.casting.Bfloat16Cast",
        lambda pred: f"CAST({pred})",
    )
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.normalization.InputsAndResiduals",
        lambda pred, **kw: f"NORM({pred})",
    )
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.autoregressive.Predictor",
        lambda pred, gradient_checkpointing: f"PRED({pred})",
    )

    dummy_ckpt = DummyCKPT()
    diffs = mean = std = xr.Dataset()
    constructor = build_model(dummy_ckpt, diffs, mean, std)
    result = constructor({"a": 1}, {"b": 2})
    assert result == "PRED(NORM(CAST(GRAPHCAST_INSTANCE)))"


@mock.patch("pkg.forecast.run_forecast.xr.Dataset.to_netcdf", lambda self, path: None)
@mock.patch("pkg.forecast.run_forecast.gcs.upload_file")
@mock.patch("pkg.forecast.run_forecast.xr.open_dataset")
@mock.patch("pkg.forecast.run_forecast.rollout.chunked_prediction")
@mock.patch("pkg.forecast.run_forecast.data_utils.extract_inputs_targets_forcings")
@mock.patch("pkg.forecast.run_forecast.load_normalization_data")
@mock.patch("pkg.forecast.run_forecast.get_model_checkpoint")
def test_run_forecast_happy_path(
    ...
    # Fake extract_inputs_targets_forcings
    fake_inp = np.zeros((1,))
    fake_targ = np.ones((1,))
    ...
    result = run_forecast(
        ...
        input_data_or_path="irrelevant.nc",
        output_name="myout",
        upload_to_gcs=True,
...
def test_run_forecast_no_upload(tmp_path, monkeypatch):
    ...
    bucket, prefix = object(), "graphcast/"
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.get_model_checkpoint",
        lambda name: (ckpt, bucket, prefix),
    )
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.load_normalization_data",
        lambda b, p: (xr.Dataset(), xr.Dataset(), xr.Dataset()),
    )
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.xr.open_dataset",
        lambda path: mock.MagicMock(load=lambda: xr.Dataset({"X": ("time",[0.])})),
    )
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.data_utils.extract_inputs_targets_forcings",
        lambda *a, **kw: (np.zeros((1,)), np.zeros((1,)), np.zeros((1,))),
    )
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.rollout.chunked_prediction",
        lambda *a, **kw: xr.Dataset({"Y": ("time",[0.])}),
    )
    # Stub out to_netcdf & upload_file
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.xr.Dataset.to_netcdf",
        lambda self, path: None,
    )
    called = []
    monkeypatch.setattr(
        "pkg.forecast.run_forecast.gcs.upload_file",
...
def test_run_forecast_missing_stats_bucket(monkeypatch):
    ...
    assert "normalization statistics" in str(exc.value).lower()
